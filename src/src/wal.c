...
#if (AWS_S3_RECV == 2) 
	// (jhpark): write added part of WAL file.
	//rc = sqlite3OsWrite(wal_fd, pContent, iAmt, iOffset);
	memcpy(aws_ptr +aws_off, pContent, iAmt);
	aws_off += iAmt;
#endif
  return rc;
}
...
int sqlite3WalCallback(Wal *pWal){
	...
	// (jhpark): aws command
	if (pWal && pWal->zWalName) {

#if (AWS_S3_RECV == 1) 
		//fprintf(stderr, "WAL FILE NAME: %s\n", pWal->zWalName);
		// upload the wal file 
		sprintf(command, "aws s3api put-object --bucket ku-express-test--use1-az6--x-s3 --key wal-file/sqlite.wal --body \"%s\" > /dev/null", pWal->zWalName);
		if (system(command) != 0) {
		...
		cur_aws_off += aws_off;
		aws_off=0;
		ftruncate(aws_fd, (1024*1024));
		memset(aws_ptr,0x0,(1024*1024));
